...
(() => {
  ...
  /* ---------- «хром»: счётчик, точки, прогресс, иконка кнопки ---------- */
  ...
  function updateReveals() {
    ...
    active.querySelectorAll('.reveal:not(.in)').forEach((b) => {
      if (b.getBoundingClientRect().top < line) b.classList.add('in');
    });
  }
  ...
  function syncScrollHint() {
    const active = slides[idx];
    const atTop = active ? active.scrollTop < 60 : true;
    body.classList.toggle('show-scroll-hint', atTop);
  }

  /* ---------- переход к слайду n в направлении dir ---------- */
  function goTo(n, dir = 1) {
    ...
    // новый въезжает с нужной стороны, скролл — наверх
  ...
  /* ---------- прокрутка внутри активного слайда ---------- */
  slides.forEach((s) => {
  ...
  window.addEventListener('resize', requestReveals, { passive: true });
